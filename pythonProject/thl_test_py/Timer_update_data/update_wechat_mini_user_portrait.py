...
# @Author : Think
# @Date : 2023-04-10
# @File_name : update_wechat_mini_user_portrait.py
...
source_table = 'wechat_mini_user_portrait'

target_table = 'wechat_mini_user_portrait'

record_table = 'wechat_mini_user_portrait_update_record'

record_max_id_sql = "select * from %s where `date` = date_sub(date(now()),INTERVAL 1 DAY)" % record_table


# 主要更新调用函数
def update_wechat_mini_user_portrait_fun(source_db_dict, target_db_dict,
...
if __name__ == "__main__":
    update_wechat_mini_user_portrait_fun(bjlmdc_db_info, thltest_db_info,
                                         source_table, target_table,
                                         record_table, record_max_id_sql)
else:
    pass
